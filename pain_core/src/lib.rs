...
impl SpatialGrid {
    ...
    pub fn insert(&mut self, mut molecule: Molecule) -> u64 {
        ...
        self.grid.entry(cell_coords).or_insert_with(Vec::new).push(id);
    ...
    pub fn update_molecule_pos(&mut self, id: u64, new_pos: Vector2<f32>) {
        if let Some(mol) = self.molecules.get_mut(&id) {
            ...
            self.grid.entry(new_cell_coords).or_insert_with(Vec::new).push(id);
        }
    }
...
impl SimulationState {
    ...
    pub fn add_yeast(&mut self) {
        if !self.yeast_added {
            ...
            for _ in 0..yeast_amount {
                ...
                let sugar_x = rand::thread_rng().gen_range((x - 20.0).max(0.0)..(x + 20.0).min(self.width));
                let sugar_y = rand::thread_rng().gen_range((y - 20.0).max(0.0)..(y + 20.0).min(self.height));
    ...
    pub fn apply_force_to_region(&mut self, center: Vector2<f32>, radius: f32, force: Vector2<f32>) {
    ...
    fn form_disulfide_bridges(&mut self) {
        ...
        for mol in self.grid.get_all_molecules() {
            if let MoleculeType::Glutenin { has_free_thiol: true } = mol.mol_type {
                ...
                for neighbor in neighbors {
                    ...
                    if let MoleculeType::Glutenin { has_free_thiol: true } = neighbor.mol_type {
                        let dist = (mol.pos - neighbor.pos).magnitude();

                        // Check if they're close enough to react
                        if dist < 8.0 {  // Reaction distance threshold
                            ...
                            if rand::thread_rng().gen::<f32>() < reaction_prob * 0.1 { // Scale down frequency
        ...
        for bond in new_bonds {
            ...
            if !self.bonds.iter().any(|b| {
                ...
                (b.molecule_a_id == bond.molecule_b_id && b.molecule_b_id == bond.molecule_a_id)
            }) {
        ...
        for id in mol_ids_to_update {
            if let Some(mol_mut) = self.grid.get_molecule_mut(id) {
                if let MoleculeType::Glutenin { ref mut has_free_thiol } = mol_mut.mol_type {
            ...
            if let MoleculeType::Yeast = mol.mol_type {
                ...
                for neighbor in neighbors {
                    ...
                    if matches!(neighbor.mol_type, MoleculeType::Sugar) {
                        // Calculate distance
                        let dist = (mol.pos - neighbor.pos).magnitude();
                        if dist < 5.0 { // Within reaction distance
                            // Consume the sugar
                            ...
                            if rand::thread_rng().gen::<f32>() < 0.01 * metabolism_rate * dt {
                                ...
                                let co2_molecule = Molecule::new(MoleculeType::CO2, co2_pos, co2_vel);
                                ...
                                if rand::thread_rng().gen::<f32>() < 0.3 {
                                    ...
                                    let ethanol_molecule = Molecule::new(MoleculeType::Ethanol, ethanol_pos, ethanol_vel);
